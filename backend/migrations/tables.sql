...
CREATE TABLE Appointments(
    id serial PRIMARY KEY,       --: Unique value for each appointment
...
CREATE TABLE Day(
    id   serial primary key not null,    --: Unique value for each day
    name  varchar(20) 	                  --: Name of the day (Monday to Friday)
);
	
CREATE TABLE Interviews(
    id  serial primary key , --: Unique value for each interview
...
CREATE TABLE Interviewers(
    id serial primary key , --: Unique value for each interviewer
...
CREATE TABLE Available_Interviewers(
    id  serial primary key ,  --: Unique value for each available interviewer
    interviewer_id   integer not null, --: Id of the interviewer
